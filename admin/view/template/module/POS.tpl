    ...
    <div class="content">
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <table id="payment_type_table" class="list">
		  ...
		  <tbody id="payment_type_list">
            <tr class='filter' id="payment_type_add">
              <td class="left" width="70%"><input type="text" name="payment_type" id="payment_type" size="50" value="" onkeypress="return addPaymentOnEnter(event)" /></td>
		...
		<tr id="<?php echo 'payment_type_'.$payment_type_row_no; ?>">
			<td class="left" width="70%"><?php echo $payment_type; ?></td>
			<td class="right" width="30%"><a onclick="deletePaymentType('<?php echo 'payment_type_'.$payment_type_row_no; ?>');" class="button" size=2><?php echo $button_remove; ?></a></td>
			<input type="hidden" name="POS_payment_types[<?php echo $payment_type_row_no; ?>]" value="<?php echo $payment_type; ?>"/>
		</tr>
		<?php $payment_type_row_no ++; }} ?>
          </tbody>
        </table>        <table id="page_display" class="list">          <thead>			<tr>				<td class="left" style="background-color: #E7EFEF;"><?php echo $text_display_setting; ?></td>			</tr>          </thead>		  <tbody>			<tr><td class="left">				<input type="checkbox" name="display_once_login" value="0" <?php if($display_once_login=='1') { ?>checked="checked"<?php } ?> />&nbsp;				<?php echo $text_display_once_login; ?>			</td></tr>          </tbody>        </table>
      </form>
    </div>
	</div>
   </div>
