...
<!--
              ...
              <td class="left"><?php if ($sort == 'o.date_modified') { ?>
                ...
                <?php } ?></td>
-->
              <td class="left"><?php if ($sort == 'o.shipping_method') { ?>
...
<!--
    ...
    -->                                                                          
          ...
          <tbody>
            <tr class="filter">
              <td>
                <select name="processing_status_id_selector" id="processing_status_id_selector" style="width:35px;">
                ...
                <script>
                  $(document).ready(function () {
                    $('#processing_status_id_selector').change(function () {
                      var selectedValue = $('#processing_status_id_selector :selected').val();
                      $(".processing_checkbox input").prop("checked", false);
                      $(".processing_id" + selectedValue + " input").prop("checked", true);
                    });
                  });
                </script>
              </td>
...
<!--              <td><input type="text" name="filter_date_modified" value="<?php echo $filter_date_modified; ?>" size="12" class="date" /></td> -->
              <td><select name="filter_shipping_method">
                 ...
                 <?php foreach($shipping_methods as $method) { ?>
                  <option value="<?php echo $method['shipping_method']; ?>"><?php echo $method['shipping_method']; ?></option>
                 <?php } ?>
                  </select>
              </td>
...
<!--              <td><select name="filter_payment_method">
    ...
    -->                                                                                                                              
            ...
            <tr>
              <td style="text-align: center;" class="processing_checkbox processing_id<?php echo $order['order_processing_status_id']; ?>"><?php if ($order['selected']) { ?>
                <input type="checkbox" name="selected[]" value="<?php echo $order['order_id']; ?>" checked="checked" />
                <?php } else { ?>
                <input type="checkbox" name="selected[]" value="<?php echo $order['order_id']; ?>" />
                <?php } ?></td>
              <td class="right processing_id<?php echo $order['order_processing_status_id']; ?>"><?php echo $order['order_id']; ?></td>
              <td class="left processing_id<?php echo $order['order_processing_status_id']; ?>"><?php echo $order['customer']; ?></td>
              <td class="left processing_id<?php echo $order['order_processing_status_id']; ?>"><?php echo $order['status']; ?></td>
              <td class="right processing_id<?php echo $order['order_processing_status_id']; ?>"><?php echo $order['total']; ?></td>
              <td class="left processing_id<?php echo $order['order_processing_status_id']; ?>"><?php echo date('d M Y', strtotime(str_replace('/','-',$order['date_added']))); ?></td>
              <!--<td class="left"><?php echo $order['date_modified']; ?></td>-->
              <td class="left processing_id<?php echo $order['order_processing_status_id']; ?>"><?php echo $order['shipping_method']; ?></td>
              <td class="right processing_id<?php echo $order['order_processing_status_id']; ?>"><?php foreach ($order['action'] as $action) { ?>
                <!-- -->
