...
<div id="content">
	...
	<div class="box">
		<div class="heading">
			...
			<div class="buttons">
				<?php if (!$this->config->get('config_dis_order_button') || $this->config->get('config_dis_user_group') != $this->user->getId()) { ?>
					<a id="add_order" title="Create Order" class="button"><span><?php echo $button_add_order; ?></span></a>
				<?php } ?>
				<?php if (!$this->config->get('config_dis_quote_button') || $this->config->get('config_dis_user_group') != $this->user->getId()) { ?>
					<a id="add_quote" title="Create Quote" class="button"><span><?php echo $button_add_quote; ?></span></a>
				<?php } ?>
				<?php if (!$this->config->get('config_dis_customer_button') || $this->config->get('config_dis_user_group') != $this->user->getId()) { ?>
...
<!--					<a id="export_orders" style="display: none !important;" title="Export Orders" alt="Export Orders" class="button2"><?php echo $button_export; ?></a>
					...
					<a id="print_orders" style="display: none !important;" title="Print Orders" alt="Print Orders" class="button2"><?php echo $button_print; ?></a>
							...
							<?php if ($orders) { ?>
								...
								<?php foreach ($orders as $order) { ?>
									...
									<?php } else { ?>
										<tr>
									<?php } ?>
										<td class="data-center"><input type="checkbox" class="selected" name="selected[<?php echo $order['order_id']; ?>]" value="<?php echo $order['order_id'];  ?>" title="<?php echo $order['order_id']; ?>" /></td>
										<?php if ($this->config->get('config_order_entry_store')) { ?>
			...
			<div id="customer_info" style="display: none !important;">
				...
				<form id="customer_form" name="customer_form" enctype="multipart/form-data" method="post">
					...
					<div class="customer_right" style="display: none !important;">
						...
						<div style="float: left; width: 100%; margin-bottom: 6px;">
							<?php if (!$this->config->get('config_dis_dropship') || $this->config->get('config_dis_user_group') != $this->user->getId()) { ?>
...
<script type="text/javascript"><!--
	...
	$(document).ready(function() {
		...
		$('#order_buttons a').on('click', function() {
			if ($(this).attr('rel') == 'Delete Order' || $(this).attr('rel') == 'Delete Quote') {
			...
			} else if ($(this).attr('rel') == 'Print Order' || $(this).attr('rel') == 'Print Quote') {
				var new_window = window.open();
				if ($(this).attr('rel') == 'Print Order') {
					var dataString = 'order_id=' + $(this).attr('name') + '&type=print';
				} else {
				...
				if ($(this).attr('rel') == 'Email Order') {
// Future development for allowing a prompt email address to be entered.
//          var mailTo = prompt('Email Address?\n\nEnter a full email address - user@domain.com\n\nLeave blank to use the email account associated with the order.');
		...
		$('#add_order').on('click', function() {
			if ($(this).attr('title') == 'Create Order' || $(this).attr('title') == 'Create Quote') {
				$('#please_wait').show();
				$.ajax({
					url: 'index.php?route=sale/order_entry/startOrder&token=<?php echo $token; ?>',
					type: 'GET',
					dataType: 'json',
					success: function(json) {
						$('#please_wait').hide();
		...
		$('#product_info').on('keyup', '.qty', function() {
			...
			if (cunit > 0) {
				var new_total = parseFloat($('input[name=\'price\']').val()) * parseInt($('input[name=\'qty\']').val()) * parseInt(cunit);
			} else {
				var new_total = parseFloat($('input[name=\'price\']').val()) * parseInt($('input[name=\'qty\']').val());
			}
		...
		$('#product_info').on('keyup', '.qty2', function() {
			...
			if (cunit > 0) {
				var new_total = parseFloat($('#price-' + row).val()) * parseInt($('#quantity-' + row).val()) * parseInt(cunit);
			} else {
				var new_total = parseFloat($('#price-' + row).val()) * parseInt($('#quantity-' + row).val());
			}
		...
		function updateProduct(row) {
			var dataString = 'key=' + $('#product_key-' + row).val() + '&name=' + $('#product_name-' + row).val() + '&model=' + $('#model-' + row).val() + '&qty=' + $('#quantity-' + row).val() + '&price=' + $('#price-' + row).val();
			<?php if ($this->config->get('config_prod_tax')) { ?>
			...
			<?php } ?>
			<?php if ($this->config->get('config_prod_location')) { ?>
				dataString += '&location=' + $('#product_location-' + row).val();
			<?php } ?>
			<?php if ($this->config->get('config_prod_sku')) { ?>
				dataString += '&sku=' + $('#product_sku-' + row).val();
			<?php } ?>
			<?php if ($this->config->get('config_prod_upc')) { ?>
				dataString += '&upc=' + $('#product_upc-' + row).val();
			<?php } ?>
			<?php if ($this->config->get('config_prod_weight')) { ?>
		...
		$('#comments_info').on('blur', '#note', function() {
			if ($('#note').val() != '') {
				$.ajax({
		...
		$('#total_info').on('click', '#process_order', function() {
			if ($('#customer_shipping').val() == '') {
				...
				return false;
			} else if (parseInt($('#products_count').val()) < 1) {
				...
				if (($('#payment').val() == 'pp_pro' || $('#payment').val() == 'pp_pro_uk' || $('#payment').val() == 'pp_payflow_pro' || $('#payment').val() == 'pp_pro_pf' || $('#payment').val() == 'authorizenet_aim' || $('#payment').val() == 'authorizenet_aim_simple' || $('#payment').val() == 'eprocessingnetwork' || $('#payment').val() == 'sagepay_direct' || $('#payment').val() == 'intuit_qbms' || $('#payment').val() == 'perpetual_payments' || $('#payment').val() == 'moneris_api' || $('#payment').val() == 'sagepay_server' || $('#payment').val() == 'usaepay_server' || $('#payment').val() == 'paymentsense_direct') && $('input[name=\'order_paid\']').val() == '0') {
					$('#credit_card').show();
			...
			select: function(event, ui) {
				...
				$.ajax({
					...
					success: function(json) {
						...
						$('input[name=\'price\']').val(json.price);
						if (json.cunit_qty) {
							var prod_price = parseFloat(json.price) * parseInt(json.cunit_qty);
							$('input[name=\'cunit_qty\']').val(json.cunit_qty);
						} else {
							var prod_price = parseFloat(json.price) * parseInt($('input[name=\'qty\']').val());
						}
