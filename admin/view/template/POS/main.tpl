...
<div id="oc_content" style="padding: 15px; ">
	...
	<div id="divWrap" class="top_wrapper" style="height:690px; width:1024px; overflow: hidden;">
		<div style="height: 75px;">
			<div style="float: left; margin-left: 30px; margin-top: 15px; "><img src="view/image/POS/logo.png" alt=""/></div>
			...
			<div id="order_info_and_actions" style="display: <?php echo $display_order_content; ?>">
				<div id="order_general" class="pos_content" style="height: 38px;">
					<table class="pos_list" style="height: 100%;">
						<tr>
							<td align="center" style="font-weight: bold"><?php echo $column_order_id; ?>:</td>
							...
							<td align="center">
								...
								<?php foreach ($order_statuses as $order_status_next) { ?>
							...
							<td align="center" id="customer_name_td" style="vertical-align: middle;"><a id="general_customer_name" onclick="showCustomerContent()"><?php echo $customer; ?></a><?php if ($customer_id > 0) { ?>&nbsp&nbsp;<img id="detach_customer_img" style="vertical-align: middle;" src="view/image/POS/minus_off.png" onclick="detachCustomer();" /><?php } ?></td>
				...
				<div style="margin-top: 15px;">
					<div id="order_product_content" style="display: block; ">
						<div class="pos_content" style="float: right; width: 360px; height: 538px; margin-right: 10px; padding: 5px;">
							<div style="height: 500px; overflow: hidden;">
								<div id="tabs" class="htabs" style="padding: 5px;"><a href="#tab_product_search"><?php echo $tab_product_search; ?></a><a href="#tab_product_browse"><?php echo $tab_product_browse; ?></a><a href="#tab_product_details"><?php echo $tab_product_details; ?></a></div>
								...
								<div id="tab_product_browse" style="padding: 5px;">
									<div style="text-align: center;"><?php echo $text_not_available; ?></div>
								</div>
						...
						<div class="pos_content" style="margin-left: 10px; margin-right: 390px; height: 538px; padding: 5px;">
							<div class="pos_wrapper" id="order_product_list_and_actions" style="oveflow: hidden; margin: 5px;">
									...
									<table class="list" id="order_product_list">
										...
										<?php foreach ($products as $product) { ?>
											<tr id="product-row<?php echo $product_row; ?>">
												...
												<td class="right"><?php echo $product['price_text']; ?></td>
								...
								<div style="position: relative; height: 70px; ">
									<table class="pos_form_table">
										<tbody>
											...
											<?php $lastTotal = end($totals); ?>
											<tr id="total_tr">
												<td class="center" width="30%"><span style="font: bold 20px Arial, Helvetica, sans-serif;"><?php echo $lastTotal['title']; ?>:</span></td>
												<td class="center" width="20%" id="payment_total" style="background-image: url('view/image/POS/more_down.png'); background-position: right bottom; background-repeat:no-repeat;"><span style="font: bold 20px Arial, Helvetica, sans-serif;"><?php echo $lastTotal['text']; ?></span></td>
							...
							<div id="order_payment_list" style="overfolow: auto; overflow-x: hidden; height: 163px; margin-left: 5px; margin-right: 5px; margin-top: 10px; padding: 5px; border-top: 1px solid #CCCCCC;  border-left: 1px solid #CCCCCC; border-right: 1px solid #CCCCCC; border-bottom: 1px solid #CCCCCC;">
								<table class="list">
									<thead>
										<tr>
					...
					<div class="pos_content" id="order_customer_content" style="display: none; overflow: auto; overflow-x: hidden; height: 530px; margin-left: 10px; margin-right: 10px;">
						...
						<div id="customer_customer" style="display:none; margin: 10px; position: relative;">
							<input type="hidden" name="customer_customer_id" value="<?php echo $customer_id; ?>" />
							<?php $address_row = 1; ?>
							...
							<div id="customer_buttons" style="clear: both; float: left;">
								<a id="customer_button_save" class="button" style="margin-left: 40px;"><?php echo $button_save; ?></a>&nbsp;&nbsp;&nbsp;
								<a id="customer_button_cancel" class="button"><?php echo $button_cancel; ?></a>
							</div>
...
function getOrderList() {
	if ($('#order_list').is(':visible')) {
		...
		// get the list of order and render the order list div
...
function selectOrder(anchor, href) {
	...
	$.ajax({
		...
		success: function(html) {
			...
			$('#vtabs a').tabs();
			moveSelect(-1, 0);
			showMessage('pos_success', '<?php echo $text_order_ready; ?>', null);
		}
	});
};
...
function detachCustomer() {
	...
	$.ajax({
		...
		success: function(json) {
			if (json['success']) {
				$('#order_customer input[name=\'customer\']').val(json['firstname']+' '+json['lastname']);
				$('#order_customer input[name=\'customer_id\']').val(json['customer_id']);
				$('#order_customer input[name=\'customer_group_id\']').val(json['customer_group_id']);
				$('#order_customer input[name=\'firstname\']').val(json['firstname']);
				$('#order_customer input[name=\'lastname\']').val(json['lastname']);
				$('#order_customer input[name=\'email\']').val(json['email']);
				$('#order_customer input[name=\'telephone\']').val(json['telephone']);
				$('#order_customer input[name=\'fax\']').val(json['fax']);
...
$('#button_new_order').live("click", function() {
	$.ajax({
		url: 'index.php?route=module/POS/createEmptyOrder&token=<?php echo $token; ?><?php if(isset($store_id)) { echo '&store_id='.$store_id; } ?>',
		type: 'post',
		beforeSend: function() {
			removeMessage();
			showMessage('pos_attention', null, null);
...
$('input[name=\'order_customer_name\']').live('focus', function() {
	$(this).catcomplete({
		delay: 500,
		...
		select: function(event, ui) { 
			...
			$('#customer_name_td').html('<a onclick="showCustomerContent()">' + ui.item['label'] + '</a>');
...
<?php if (isset($customer_groups)) { foreach ($customer_groups as $customer_group) { ?>
...
<?php 
	if (isset($customer_customer_groups)) {
	foreach ($customer_customer_groups as $customer_customer_group) { ?>
	customer_group[<?php echo $customer_customer_group['customer_group_id']; ?>] = [];
	customer_group[<?php echo $customer_customer_group['customer_group_id']; ?>]['company_id_display'] = '<?php echo $customer_customer_group['company_id_display']; ?>';
	customer_group[<?php echo $customer_customer_group['customer_group_id']; ?>]['tax_id_display'] = '<?php echo $customer_customer_group['tax_id_display']; ?>';
<?php }} ?>	
	if (customer_group[this.value]) {
		if (customer_group[this.value]['company_id_display'] == '1') {
			$('.company-id-display').show();
...
$('#button_custom_cancel, #customer_button_cancel').live('click', function() {
		...
		success: function(json) {
			...
			if (json['success']) {
				...
				if (json['customer_info']) {
					...
					$('select[name=\'customer_newsletter\']').val(json['customer_info']['newsletter']);
					$('select[name=\'customer_newsletter\']').change();
					$('select[name=\'customer_customer_group_id\']').val(json['customer_info']['customer_group_id']);
					$('select[name=\'customer_status\']').val(json['customer_info']['status']);
...
$('#order_status').change(function() {
	var data = {'order_id':parseInt($('#order_id').text(), 10), 'order_status_id':$(this).val()};
	$.ajax({
...
function saveCustomer() {
	data = '#customer_customer input[type=\'text\'], #customer_customer input[type=\'hidden\'], #customer_customer input[type=\'password\'], #customer_customer input[type=\'radio\']:checked, #customer_customer input[type=\'checkbox\']:checked, #customer_customer select, #customer_customer textarea';
	$.ajax({
		url: 'index.php?route=module/POS/save_customer&token=<?php echo $token; ?>',
		type: 'post',
		data: $(data),
		dataType: 'json',
		beforeSend: function() {
...
function checkAndSaveOrder(eleId, quantity) {
	if (eleId == 'button_product') {
		...
		prodQty = parseFloat(prodQty.replace(/[^0-9-.]/g, ''));
	...
	if (eleId == 'button_plus' || eleId == 'button_minus' || eleId == 'button_delete' || eleId == 'button_equal') {
		...
		indexQty = parseInt($('#product tr:eq('+index+')').find('td').eq(3).text());
	...
	<?php if (isset($currency_code)) { ?>
	data['currency_code'] = '<?php echo $currency_code; ?>';
	<?php } ?>
	<?php if (isset($currency_value)) { ?>
	data['currency_value'] = '<?php echo $currency_value; ?>';
	<?php } ?>
	data['customer_id'] = '0';
	...
	<?php if (isset($customer_group_id) && $customer_group_id > 0) { ?>
	data['customer_group_id'] = '<?php echo $customer_group_id; ?>';
	<?php } ?>
	<?php if (isset($shipping_country_id)) { ?>
	data['shipping_country_id'] = '<?php echo $shipping_country_id; ?>';
	<?php } ?>
	<?php if (isset($shipping_zone_id)) { ?>
	data['shipping_zone_id'] = '<?php echo $shipping_zone_id; ?>';
	<?php } ?>
	<?php if (isset($payment_country_id)) { ?>
	data['payment_country_id'] = '<?php echo $payment_country_id; ?>';
	<?php } ?>
	<?php if (isset($payment_zone_id)) { ?>
	data['payment_zone_id'] = '<?php echo $payment_zone_id; ?>';
	<?php } ?>

	data['store_id'] = '<?php echo $store_id; ?>';
	
	$.ajax({
		url: '<?php echo $store_url; ?>index.php?route=POS/POS&token=<?php echo $token; ?>',
		type: 'post',
		data: data,
		dataType: 'json',	
		beforeSend: function() {
		...
		success: function(json) {
			...
			if (json['success'] && data['action'] == 'insert' && !json['order_product']) {
				...
				$.ajax({
					url: 'index.php?route=module/POS/modifyOrder&token=<?php echo $token; ?>',
					type: 'post',
					data: saveData,
					dataType: 'json',
					success: function(save_json) {
						...
						if (eleId == 'button_plus' || eleId == 'button_minus' || eleId == 'button_equal') {
							...
							if (json['order_product']) {
								if (json['order_product']['action'] == 'modify') {
									...
									new_row_num = $('#product tr').length -1;
									...
									if (json['order_product']['option']) {
											for (i = 0; i < json['order_product']['option'].length; i++) {
									...
									html +=		' <input type="hidden" name="order_product[' + new_row_num + '][order_option][' + i + '][value]" value="' + json['order_product']['option'][i]['value'] + '" />';
									html +=		' <input type="hidden" name="order_product[' + new_row_num + '][order_option][' + i + '][type]" value="' + json['order_product']['option'][i]['type'] + '" /></td>';
											}
									}
									...
									html += '<td class="right">' + json['order_product']['price_text'] + '</td>';
						...
						if (json['order_total'] != '') {
							...
							for (i = 0; i < json['order_total'].length; i++ ) {
								...
								if (i < json['order_total'].length-1) {
							...
							$('#total_tr td:eq(0) span').text(json['order_total'][i-1]['title']);
							$('#total_tr td:eq(1) span').text(json['order_total'][i-1]['text']);
						} else {
							html  = '</tr>';
...
$('#tendered_amount').keydown(function(event) {
...
function addPayment() {
	...
	dueAmount = parseFloat(dueAmount.replace(/[^0-9-.]/g, ''));
	if (dueAmount <= 0) {
	...
	$.ajax({
		url: 'index.php?route=module/POS/addOrderPayment&token=<?php echo $token; ?>&order_payment_id='+order_payment_id+'&order_id='+order_id+'&payment_type='+type+'&tendered_amount='+amount+'&payment_note='+note,
		dataType: 'json',
		beforeSend: function() {
...
function calcDueAmount() {
	...
	totalNum = $('#total tr').length;
	var totalAmount = 0;
	if (totalNum > 0) {
		var totalText = $('#total-row'+(totalNum-1)).find("td").eq(1).text();
		totalAmount = parseFloat(totalText.replace(/[^0-9-.]/g, ''));
	}
	...
	for (i = 1; i < rows.length; i++) {
		...
		rowAmount = parseFloat(rowAmount.replace(/[^0-9-.]/g, ''));
		totalPaid += rowAmount;
	}
	...
	if (totalDue < 0) {
		...
		$('#tendered_amount').attr('value', totalDue);
	}
	if (totalDue <= 0) {
