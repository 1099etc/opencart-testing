...
$this->data['alerts'] = $this->model_sale_alert->getOrderAlerts($order_id);
      ...
      if($order_info['sales_agent'] == '') { $order_info['sales_agent'] = 'Web Order'; }

      $this->data['sales_agent'] = $order_info['sales_agent'];
	...
	public function upload() {
  	...
  	public function invoice() {
		...
		foreach ($orders as $order_id) {
          ...
          if(strpos($product['model'], "1099-FormsFiler")) {
...
$Q = $this->db->query("SELECT " . DB_PREFIX . "product_option_value.price FROM " . DB_PREFIX . "product_option_value, " . DB_PREFIX . "option_value_description WHERE " . DB_PREFIX . "product_option_value.option_value_id = " . DB_PREFIX . "option_value_description.option_value_id and " . DB_PREFIX . "product_option_value.product_option_id = " . $O['product_option_id'] . " and " . DB_PREFIX . "option_value_description.name = 'AMS Payroll'");
              ...
              if(stripos($O['name'], "E-File Direct") !== false) {
                if(stripos($O['value'], "No Thank You") !== false) {
                  $tmp_option_data[2]['value'] = "No Thank You";
                  $tmp_option_data[2]['name'] = 'E-File Direct';
                  $tmp_option_data[2]['product_option_value_id'] = $O['product_option_value_id'];
                  $tmp_option_data[2]['product_option_id'] = $O['product_option_id'];
                  $tmp_option_data[2]['type'] = $O['type'];
                  $tmp_option_data[2]['price'] = $O['price'];
                }
                else {
              ...
              if(stripos($O['name'], "Forms Filer Plus") !== false) {
                if(stripos($O['value'], "No Thank You") !== false) {
                  $tmp_option_data[3]['value'] = "No Thank You";
                  $tmp_option_data[3]['name'] = 'Forms Filer Plus';
                  $tmp_option_data[3]['product_option_value_id'] = $O['product_option_value_id'];
                  $tmp_option_data[3]['product_option_id'] = $O['product_option_id'];
                  $tmp_option_data[3]['type'] = $O['type'];
                  $tmp_option_data[3]['price'] = $O['price'];
                }
                else {
        ...
        elseif($order_info['order_paid'] == 1) {
          foreach($total_data as $t => $total) {
            if($total['code'] == 'total') {
              $unpaidBalanceText  = $total['text'];
              $unpaidBalanceValue = $total['value'];
          ...
          $unpaidBalanceText = 0;
          $unpaidBalanceValue= 0;
        }

				$this->data['orders'][] = array(
					'order_id'	         => $order_id,
					'invoice_no'         => $invoice_no,
					'invoice_date'       => date('d M Y', strtotime($order_info['invoice_date'])),
          'po_number'          => $order_info['po_number'],
          'order_paid'         => $order_info['order_paid'],
          'unpaid_balance'     => $unpaidBalanceValue,
