...
class ControllerSaleCustomer extends Controller { 
	private $error = array();
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$customer_id = $this->model_sale_customer->addCustomer($this->request->post);
			      if(isset($this->request->post['private'])) { $this->model_sale_customer->editCustomerPrivacy($customer_id, $this->request->post['private']); }
            if(isset($this->request->post['payroll_states'])) { $this->model_sale_customer->editCustomerPayrollStates($customer_id, $this->request->post['payroll_states']); }
      ...
      if(isset($this->request->post['payroll_states'])) {
        $payroll_states = $this->request->post['payroll_states'];
      }
      else { $payroll_states = ''; }

      $this->request->post['payroll_states'] = $payroll_states;
   
			if($this->model_sale_customer->editCustomer($this->request->get['customer_id'], $this->request->post)) {
  		  if(isset($this->request->post['private'])) { $this->model_sale_customer->editCustomerPrivacy($this->request->get['customer_id'], $this->request->post['private']); }
	  		if(isset($this->request->post['payroll_states'])) { $this->model_sale_customer->editCustomerPayrollStates($this->request->get['customer_id'], $this->request->post['payroll_states']); }
      }
...
if(isset($this->request->get['customer_id'])) {
  ...
  $this->data['alerts'] = $this->model_sale_alert->getCustomerAlerts($this->request->get['customer_id']);
}

if(isset($customer_info)) {
  $this->data['private'] = $customer_info['private'];
  $this->data['payroll_states'] = $customer_info['payroll_states'];
}

      if (isset($this->request->post['private'])) {
          $this->data['private'] = $this->request->post['private'];
      ...
      if (isset($this->request->post['payroll_states'])) {
          $this->data['payroll_states'] = $this->request->post['payroll_states'];
    } elseif (!empty($customer_info)) {
      $this->data['payroll_states'] = $customer_info['payroll_states'];
    } else {
          $this->data['payroll_states'] = '';
      }
