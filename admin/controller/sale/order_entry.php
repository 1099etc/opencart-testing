...
class ControllerSaleOrderEntry extends Controller {
	...
	public function index() {
		...
		$data = array(
			...
			'filter_start_paymenn_date'  => $filter_start_payment_date,
		...
		if ($results) {
			foreach ($results as $result) {
				...
				if ($result['order_status_id'] == $this->data['quote_order_status_id'] || $result['payment_code'] == "pending" || ($result['payment_code'] == "pp_link" && $result['order_paid'] ==0) || ($result['payment_code'] == "invoiced" && $result['order_paid'] == 0) || ($result['payment_code'] == "purchase_order" && $result['order_paid'] == 0)) {
				...
				$this->data['orders'][] = array(
					...
					'products'				=> $products,
					'cart_weight'			=> round($cart_weight, 2),
	...
	public function newCustomer() {
		...
		if ($result || !$this->config->get('config_require_email')) {
			...
			if ($this->config->get('config_require_email') || $this->request->post['email'] != "") {
				...
				$html .= "<br />";
				if (isset($this->session->data['store_id'])) {
					$html .= "<p>" . sprintf($this->language->get('text_email_line4'), HTTP_CATALOG . 'index.php?route=account/password', $this->session->data['store_config']['config_name']) . "</p>";
					$html .= "<br />";
			...
			$html = "";
			if (isset($this->request->post['customer_store'])) {
				$this->session->data['store_id'] = $this->request->post['customer_store'];
			} else {
				$this->session->data['store_id'] = $this->config->get('config_store_id');
			}
			$this->load->model('setting/setting');
			$this->session->data['store_config'] = $this->model_setting_setting->getSetting('config', $this->session->data['store_id']);
			$customers = $this->model_sale_order_entry->getCustomers();
			if ($customers) {
				foreach ($customers as $customer) {
	...
	public function getCustomerInfo() {
		...
		if ((isset($this->request->post['customer_id']) && $this->request->post['customer_id'] != "") || (isset($this->request->post['address_id']) && $this->request->post['address_id'] != "")) {
			...
			$this->session->data['shipping_zone_id'] = $address['zone_id'];
			$this->session->data['shipping_postcode'] = $address['postcode'];
			$this->session->data['billing_telephone'] = $customer['telephone'];
			$this->session->data['shipping_telephone'] = $customer['telephone'];
      $this->session->data['telephone'] = $customer['telephone'];
	...
	public function getProduct() {
      ...
      // Added By John
      // ***************************************************************
      $modelNumber = $product_info['model'];
      $yearDigit = substr($modelNumber, 7, 1);

      $autocompletes = $this->db->query("select concat(`key`,'', featurecode) as SN from serials_order where `key` Like '" . $yearDigit . "%'"); // Add CustomerID here
      $autocomplete_string = '';
      foreach($autocompletes->rows as $A) {
        $autocomplete_string .= "'" . $A['SN'] . "',";
      }
/////////////////////////////////////////////////
if(substr($product_info['model'],0,4) == 'upg-') {
$option_html .= "
<script>
  $(document).ready(function() {
    $('#serialBox').css('border', '2px solid red');
    $('#add_options').hide();
    
    var availableTags = [" . $autocomplete_string . "];
    $('#serialBox').autocomplete({ source: availableTags });

  });

  $('#serialBox').blur(function() {
    var errors = 0;
    var alertMessage = '';

// This prepares all the radio buttons so we can access them easily.
  var eFileRadioIDarray = new Array();
  var payrollLaserRadioIDarray = new Array();
  var formsFilerRadioIDarray = new Array();

  var eFileDirectContainerID  = document.getElementsByClassName('radioDiv')[0].id;
  var payrollLaserContainerID = document.getElementsByClassName('radioDiv')[1].id;
  var formsFilerContainerID   = document.getElementsByClassName('radioDiv')[2].id;
...
// Start validating serial box.
    if($('#serialBox').val().length != 8) {
      errors = errors + 1;
      alertMessage = alertMessage + 'The serial number must be 8 characters.\\n\\n';
    }

    if($('#serialBox').val().substr(0,1) != '" . $yearDigit . "') {
     errors = errors + 1;
     alertMessage = alertMessage + 'This year digit of the serial number is incorrect.\\n'; 
     alertMessage = alertMessage + 'Entered Year Digit - ' + $('#serialBox').val().substr(0,1) + '\\n';
     alertMessage = alertMessage + 'Product Year Digit - ' + " . $yearDigit . " + '\\n\\n';
    }
    
    var regex2char = /^[a-zA-Z]*$/;
   // if(regex2char.test($('#serialBox').val().substr(1,1)) != true) {
   //   errors = errors + 1;
   //   alertMessage = alertMessage + 'The second character of the serial number must be a letter.\\n';
   //   alertMessage = alertMessage + 'Submitted second character - ' + $('#serialBox').val().substr(1,1) + '\\n\\n';
   // }

// Get Feature codes
    var eFileFC = $('#serialBox').val().substr(5,1).toUpperCase();
    var payrollLaserFC = $('#serialBox').val().substr(6,1).toUpperCase(); 
    var formFilerFC = $('#serialBox').val().substr(7,1).toUpperCase();
    
//    alertMessage = alertMessage + 'eFileFC - ' + eFileFC + '\\n';
//    alertMessage = alertMessage + 'payrollLaserFC - ' + payrollLaserFC + '\\n';
//    alertMessage = alertMessage + 'formFilerFC - ' + formFilerFC + '\\n\\n';

    // Check each featurecode and disable the radio buttons that need to be disabled.
    switch(eFileFC) {
      case 'P':
        $('#' + eFileRadioIDarray[0]).attr('disabled',false);
        $('#' + eFileRadioIDarray[1]).attr('disabled',true);
        $('#' + eFileRadioIDarray[2]).attr('disabled',false);
        document.getElementById(eFileRadioIDarray[0]).checked = true;

        $('label[for=' + eFileRadioIDarray[0] + '] + br').show();
        $('label[for=' + eFileRadioIDarray[0] + ']').show();
        $('#' + eFileRadioIDarray[0]).show();
        $('label[for=' + eFileRadioIDarray[1] + '] + br').hide();
        $('label[for=' + eFileRadioIDarray[1] + ']').hide();
        $('#' + eFileRadioIDarray[1]).hide();
        $('label[for=' + eFileRadioIDarray[2] + '] + br').show();
        $('label[for=' + eFileRadioIDarray[2] + ']').show();
        $('#' + eFileRadioIDarray[2]).show();
      break;
      case 'M':
        $('#' + eFileRadioIDarray[0]).attr('disabled',true);
        $('#' + eFileRadioIDarray[1]).attr('disabled',false);
        $('#' + eFileRadioIDarray[2]).attr('disabled',true);
        document.getElementById(eFileRadioIDarray[1]).checked = true;

        $('label[for=' + eFileRadioIDarray[0] + '] + br').hide();
        $('label[for=' + eFileRadioIDarray[0] + ']').hide();
        $('#' + eFileRadioIDarray[0]).hide();
        $('label[for=' + eFileRadioIDarray[1] + '] + br').show();
        $('label[for=' + eFileRadioIDarray[1] + ']').show();
        $('#' + eFileRadioIDarray[1]).show();
        $('label[for=' + eFileRadioIDarray[2] + '] + br').hide();
        $('label[for=' + eFileRadioIDarray[2] + ']').hide();
        $('#' + eFileRadioIDarray[2]).hide();
      break;
      default:
        alertMessage = alertMessage + 'The Featurecode belonging to E-File Direct is not set properly.\\n\\n';
        errors = errors + 1;
        $('#serialBox').css('border', '2px solid red');
        $('#add_options').hide();
    }
    switch(payrollLaserFC) {
      case 'A':
        $('#' + payrollLaserRadioIDarray[0]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[1]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[2]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[3]).attr('disabled',false);
        $('#' + payrollLaserRadioIDarray[4]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[5]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[6]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[7]).attr('disabled',false);
        $('#' + payrollLaserRadioIDarray[8]).attr('disabled',true);
        document.getElementById(payrollLaserRadioIDarray[3]).checked = true;

        $('label[for=' + payrollLaserRadioIDarray[0] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[0] + ']').hide();
        $('#' + payrollLaserRadioIDarray[0]).hide();
        $('label[for=' + payrollLaserRadioIDarray[1] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[1] + ']').hide();
        $('#' + payrollLaserRadioIDarray[1]).hide();
        $('label[for=' + payrollLaserRadioIDarray[2] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[2] + ']').hide();
        $('#' + payrollLaserRadioIDarray[2]).hide();
        $('label[for=' + payrollLaserRadioIDarray[3] + '] + br').show();
        $('label[for=' + payrollLaserRadioIDarray[3] + ']').show();
        $('#' + payrollLaserRadioIDarray[3]).show();
        $('label[for=' + payrollLaserRadioIDarray[4] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[4] + ']').hide();
        $('#' + payrollLaserRadioIDarray[4]).hide();
        $('label[for=' + payrollLaserRadioIDarray[5] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[5] + ']').hide();
        $('#' + payrollLaserRadioIDarray[5]).hide();
        $('label[for=' + payrollLaserRadioIDarray[6] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[6] + ']').hide();
        $('#' + payrollLaserRadioIDarray[6]).hide();
        $('label[for=' + payrollLaserRadioIDarray[7] + '] + br').show();
        $('label[for=' + payrollLaserRadioIDarray[7] + ']').show();
        $('#' + payrollLaserRadioIDarray[7]).show();
        $('label[for=' + payrollLaserRadioIDarray[8] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[8] + ']').hide();
        $('#' + payrollLaserRadioIDarray[8]).hide();
      break;
      case 'B':
        $('#' + payrollLaserRadioIDarray[0]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[1]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[2]).attr('disabled',false);
        $('#' + payrollLaserRadioIDarray[3]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[4]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[5]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[6]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[7]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[8]).attr('disabled',true);
        ...
        $('label[for=' + payrollLaserRadioIDarray[1] + ']').hide();
        $('#' + payrollLaserRadioIDarray[1]).hide();
        $('label[for=' + payrollLaserRadioIDarray[2] + '] + br').show();
        $('label[for=' + payrollLaserRadioIDarray[2] + ']').show();
        $('#' + payrollLaserRadioIDarray[2]).show();
        $('label[for=' + payrollLaserRadioIDarray[3] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[3] + ']').hide();
        $('#' + payrollLaserRadioIDarray[3]).hide();
        $('label[for=' + payrollLaserRadioIDarray[4] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[4] + ']').hide();
        $('#' + payrollLaserRadioIDarray[4]).hide();
        $('label[for=' + payrollLaserRadioIDarray[5] + '] + br').hide();
        ...
        $('label[for=' + payrollLaserRadioIDarray[6] + ']').hide();
        $('#' + payrollLaserRadioIDarray[6]).hide();
        $('label[for=' + payrollLaserRadioIDarray[7] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[7] + ']').hide();
        $('#' + payrollLaserRadioIDarray[7]).hide();
        $('label[for=' + payrollLaserRadioIDarray[8] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[8] + ']').hide();
        $('#' + payrollLaserRadioIDarray[8]).hide();
      break;
      case 'L':
        $('#' + payrollLaserRadioIDarray[0]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[1]).attr('disabled',false);
        $('#' + payrollLaserRadioIDarray[2]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[3]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[4]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[5]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[6]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[7]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[8]).attr('disabled',false);
        document.getElementById(payrollLaserRadioIDarray[1]).checked = true;

        $('label[for=' + payrollLaserRadioIDarray[0] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[0] + ']').hide();
        $('#' + payrollLaserRadioIDarray[0]).hide();
        $('label[for=' + payrollLaserRadioIDarray[1] + '] + br').show();
        $('label[for=' + payrollLaserRadioIDarray[1] + ']').show();
        $('#' + payrollLaserRadioIDarray[1]).show();
        $('label[for=' + payrollLaserRadioIDarray[2] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[2] + ']').hide();
        $('#' + payrollLaserRadioIDarray[2]).hide();
        $('label[for=' + payrollLaserRadioIDarray[3] + '] + br').hide();
        ...
        $('label[for=' + payrollLaserRadioIDarray[7] + ']').hide();
        $('#' + payrollLaserRadioIDarray[7]).hide();
        $('label[for=' + payrollLaserRadioIDarray[8] + '] + br').show();
        $('label[for=' + payrollLaserRadioIDarray[8] + ']').show();
        $('#' + payrollLaserRadioIDarray[8]).show();
      break;
      case 'X':
        $('#' + payrollLaserRadioIDarray[0]).attr('disabled',false);
        $('#' + payrollLaserRadioIDarray[1]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[2]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[3]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[4]).attr('disabled',false);
        $('#' + payrollLaserRadioIDarray[5]).attr('disabled',false);
        $('#' + payrollLaserRadioIDarray[6]).attr('disabled',false);
        $('#' + payrollLaserRadioIDarray[7]).attr('disabled',true);
        $('#' + payrollLaserRadioIDarray[8]).attr('disabled',true);
        document.getElementById(payrollLaserRadioIDarray[0]).checked = true;

        $('label[for=' + payrollLaserRadioIDarray[0] + '] + br').show();
        $('label[for=' + payrollLaserRadioIDarray[0] + ']').show();
        $('#' + payrollLaserRadioIDarray[0]).show();
        $('label[for=' + payrollLaserRadioIDarray[1] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[1] + ']').hide();
        $('#' + payrollLaserRadioIDarray[1]).hide();
        $('label[for=' + payrollLaserRadioIDarray[2] + '] + br').hide();
        ...
        $('label[for=' + payrollLaserRadioIDarray[3] + ']').hide();
        $('#' + payrollLaserRadioIDarray[3]).hide();
        $('label[for=' + payrollLaserRadioIDarray[4] + '] + br').show();
        $('label[for=' + payrollLaserRadioIDarray[4] + ']').show();
        $('#' + payrollLaserRadioIDarray[4]).show();
        $('label[for=' + payrollLaserRadioIDarray[5] + '] + br').show();
        $('label[for=' + payrollLaserRadioIDarray[5] + ']').show();
        $('#' + payrollLaserRadioIDarray[5]).show();
        $('label[for=' + payrollLaserRadioIDarray[6] + '] + br').show();
        $('label[for=' + payrollLaserRadioIDarray[6] + ']').show();
        $('#' + payrollLaserRadioIDarray[6]).show();
        $('label[for=' + payrollLaserRadioIDarray[7] + '] + br').hide();
        $('label[for=' + payrollLaserRadioIDarray[7] + ']').hide();
        $('#' + payrollLaserRadioIDarray[7]).hide();
        $('label[for=' + payrollLaserRadioIDarray[8] + '] + br').hide();
      ...
      break;
      default:
        alertMessage = alertMessage + 'The Featurecode belonging to Software Generated Forms and AMS Payroll is not set properly.\\n\\n';
        errors = errors + 1;
        $('#serialBox').css('border', '2px solid red');
        $('#add_options').hide();
    }
    switch(formFilerFC) {
      case 'F':
        $('#' + formsFilerRadioIDarray[0]).attr('disabled',true);
        $('#' + formsFilerRadioIDarray[1]).attr('disabled',false);
        $('#' + formsFilerRadioIDarray[2]).attr('disabled',true);
        document.getElementById(formsFilerRadioIDarray[1]).checked = true;

        $('label[for=' + formsFilerRadioIDarray[0] + '] + br').hide();
        $('label[for=' + formsFilerRadioIDarray[0] + ']').hide();
        $('#' + formsFilerRadioIDarray[0]).hide();
        $('label[for=' + formsFilerRadioIDarray[1] + '] + br').show();
        $('label[for=' + formsFilerRadioIDarray[1] + ']').show();
        $('#' + formsFilerRadioIDarray[1]).show();
        $('label[for=' + formsFilerRadioIDarray[2] + '] + br').hide();
        $('label[for=' + formsFilerRadioIDarray[2] + ']').hide();
        $('#' + formsFilerRadioIDarray[2]).hide();
      break;
      case 'X':
        $('#' + formsFilerRadioIDarray[0]).attr('disabled',false);
        $('#' + formsFilerRadioIDarray[1]).attr('disabled',true);
        $('#' + formsFilerRadioIDarray[2]).attr('disabled',false);
        document.getElementById(formsFilerRadioIDarray[0]).checked = true;

        $('label[for=' + formsFilerRadioIDarray[0] + '] + br').show();
        $('label[for=' + formsFilerRadioIDarray[0] + ']').show();
        $('#' + formsFilerRadioIDarray[0]).show();
        $('label[for=' + formsFilerRadioIDarray[1] + '] + br').hide();
        $('label[for=' + formsFilerRadioIDarray[1] + ']').hide();
        $('#' + formsFilerRadioIDarray[1]).hide();
        $('label[for=' + formsFilerRadioIDarray[2] + '] + br').show();
        $('label[for=' + formsFilerRadioIDarray[2] + ']').show();
        $('#' + formsFilerRadioIDarray[2]).show();
      break;
      default:
        alertMessage = alertMessage + 'The Featurecode belonging to Forms Filer Plus is not set properly.\\n\\n';
        errors = errors + 1;
        $('#serialBox').css('border', '2px solid red');
        $('#add_options').hide();
    }
...
// Kick out Errors and change the serialEntry box color.
    if(alertMessage != '') {
      alert(alertMessage);
    }
    ...
    if(errors > 0) {
      ...
      $('#serialBox').css('border', '2px solid green');
      $('#add_options').show();
    }
  });
...
/////////////////////////////////////////////////////////////////////////////////////////////////
				...
				foreach ($options_data as $option_data) {
					if ($option_data['type'] == "select") {
						...
						foreach ($option_data['option_value'] as $option_value) {
							$option_html .= "<option value='" . $option_value['product_option_value_id'] . "'>" . $option_value['name'];
							if ($has_options) {
						...
						foreach ($option_data['option_value'] as $option_value) {
							$option_html .= "<input type='checkbox' name='option_oe[" . $option_data['product_option_id'] . "][]' value='" . $option_value['product_option_value_id'] . "' id='option-value-" . $option_value['product_option_value_id'] . "' />";
            ...
            // Added by John
            $option_html .= "<b>" . $option_data['name'] . ":</b><div class='radioDiv' id=" . $option_data['option_id'] . ">";
            $firstOne = 0;
            ////////////////
						foreach ($option_data['option_value'] as $option_value) {
              // Added by John
              if($firstOne == 0) { $checked = 'checked'; }
              else { $checked = ''; }
              ////////////////
              $option_html .= "<input class='option' type='radio' name='option_oe[" . $option_data['product_option_id'] . "]' value='" . $option_value['product_option_value_id'] . "' id='option-value-" . $option_value['product_option_value_id'] . "' ". $checked ." />";
              ...
              $option_html .= "<input type='text' name='option_oe[" . $option_data['product_option_id'] . "]' value='" . $option_data['option_value'] . "' ". $extraHTML ." />";
						...
						foreach ($option_data['option_value'] as $option_value) {
							$option_html .= "<tr>";
							$option_html .= "<td style='width: 1px;'><input type='radio' name='option_oe[" . $option_data['product_option_id'] . "]' value='" . $option_value['product_option_value_id'] . "' id='option-value-" . $option_value['product_option_value_id'] . "' /></td>";
			...
			$option_html .= "<div style='text-align: center;'><a id='add_options' class='button'><span>" . $this->language->get('button_options') . "</span></a></div>";
			$option_html .= "</form>";
		}
	...
	public function updateProduct() {
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			if ($product['key'] == $this->request->post['key'] && $update_cart == true) {
				$product_id = $product['product_id'];
				...
				if ($product['price'] != $this->request->post['price']) {
		...
		if (isset($product_id) && $this->request->post['qty'] > 0) {
			$cart_qty = 0;
	...
	public function saveNote() {
		$this->session->data['note'] = strip_tags($this->request->post['note']);
		echo json_encode("");
	}
	...
	public function processOrder() {
		...
		if (isset($this->session->data['store_id'])) {
			...
			$data['store_url'] = str_replace("admin/", "", $this->session->data['store_config']['config_url']);		
		} else {
  ...
  foreach($y as $z) {
    error_log(print_r($z, true));
      ...
      if (isset($this->session->data['shipping_method']['description'])) {
        $data['shipping_description'] = $this->db->escape($this->session->data['shipping_method']['description']);
      } else {
		...
		if (!isset($this->session->data['edit_order'])) {
			if (isset($this->session->data['payment_method'])) {
				if ($this->session->data['payment_method']['code'] == "bank_transfer") {
					...
					$data['comment']  = "<b>" . $this->language->get('text_payable') . "</b>\n\n";
					...
					$data['comment'] .= "<b>" . $this->language->get('text_payment') . "</b>\n";
				}
			}
		...
		if (!isset($this->session->data['edit_order'])) {
			...
			} elseif (isset($this->session->data['payment_method']) && $this->session->data['payment_method']['code'] == "invoiced") {
...
$success = true;
if ($this->config->get('config_invoiced_order_status_id')) {
  $order_status_id = $this->config->get('config_invoiced_order_status_id');
} else {
  $order_status_id = $data['order_status_id'];
}
$this->model_checkout_order->confirm($order_id, $order_status_id, $data['comment'], $data['notify'], $data);
if ($this->config->get('config_invoiced_paid_status')) {
  $this->model_checkout_order->markPaid($order_id);
}
$message = sprintf($this->language->get('text_order_success'), $order_id);
				...
				if ($this->config->get('config_pp_link_order_status_id')) {
			...
			if ($success) {
				...
				if (isset($this->request->post['note'])) {
					...
					if (trim($this->request->post['note']) != "") {
						$note['note'] = strip_tags($this->request->post['note']);
						$note['admin_id'] = $this->user->getId();
						$this->model_tool_admin_notes->addNote($order_id, $note);
					}
				}
	...
	public function editOrder() {
		...
		$additional_emails = $this->model_sale_order_entry->getAdditionalEmails($order_info['customer_id']);
		if ($additional_emails) {
			$this->session->data['add_emails'] = $additional_emails;
		} else {
			unset($this->session->data['add_emails']);
		}
		$this->session->data['customer_info'] = array(
	...
	public function invoice($latest_comment = '') {
		$this->load->model('setting/setting');
		$this->load->model('sale/order_entry');
		$this->load->model('sale/order');
		$orders = array();
      ...
      foreach ($total_data as $key=>$total) {
        if($total['title'] == 'Total' && $order_info['order_paid'] == 0)
        {
          $total_data[$key]['title'] = 'Amount Due';
        }
        elseif ($total['title'] == 'Total')
        {
          $total_data[$key]['title'] = 'Total Paid';
        }
      }
        ...
        if (isset($order_info['payment_company_id'])) {
          ...
          'order_paid'   => $order_info['order_paid']
				...
				//$html = $template->fetch('../../../catalog/view/theme/amsTheme/template/mail/order.tpl');
				if (isset($this->session->data['invoice_edit'])) {
					if ($order_info['email'] != "") {
		...
		if (isset($this->session->data['invoice_edit'])) {
			...
			if ((isset($this->request->get['type']) && ($this->request->get['type'] == 'email' || $this->request->get['type'] == 'email_quote')) || (isset($this->request->post['type']) && ($this->request->post['type'] == 'email' || $this->request->post['type'] == 'email_quote'))) {
				$this->session->data['success'] = sprintf($language->get('text_email_success'), $count);
				echo json_encode("");
			} else {
				$this->response->setOutput(json_encode($html));
			}
	...
	public function deleteHistory() {
		...
		if (!empty($order_histories)) {
			foreach ($order_histories as $order_history) {
				...
				$comments_html .= "<td class='data-left'>" . nl2br(html_entity_decode($order_history['comment'], ENT_QUOTES, 'UTF-8')) . "</td>";
	...
	private function productHtml() {
		...
		if ($this->cart->hasProducts() || isset($this->session->data['vouchers'])) {
			if ($this->cart->hasProducts()) {
				...
				foreach ($this->cart->getProducts() as $product) {
					...
					$product_html .= "<td class='label-left' style='background-color: " . $bg_color . ";'><input style='margin-right: 10px; vertical-align: middle;' class='product_name2' type='text' id='product_name-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' title='" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' name='product_name[" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "]' value='" . html_entity_decode($product['name'], ENT_QUOTES, 'UTF-8') . "' size='" . $field_size . "' /><a id='view_image-" . $product['product_id'] . "' title='" . $product['product_id'] . "' class='view_image'><img src='view/image/view_oe.png' border='0' /></a></td>";

					$product_html .= "<td class='label-left' style='background-color: " . $bg_color . ";'><input id='product_key-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' type='hidden' name='product_key[]' value='" . $product['key'] . "' /><input class='model2' type='text' id='model-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' title='" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' name='model[" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "]' value='" . html_entity_decode($product['model'], ENT_QUOTES, 'UTF-8') . "' size='20' /></td>";
					
					if ($this->config->get('config_prod_location')) {
						$product_html .= "<td class='label-left' style='background-color: " . $bg_color . ";'><input class='location2' id='product_location-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' title='" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' name='product_location-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' value='" . html_entity_decode($product['location'], ENT_QUOTES, 'UTF-8') . "' size='10' /></td>";
					}
					
					if ($this->config->get('config_prod_sku')) {
						$product_html .= "<td class='label-left' style='background-color: " . $bg_color . ";'><input class='sku2' id='product_sku-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' title='" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' name='product_sku-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' value='" . html_entity_decode($product['sku'], ENT_QUOTES, 'UTF-8') . "' size='10' /></td>";
					}
					
					if ($this->config->get('config_prod_upc')) {
						$product_html .= "<td class='label-left' style='background-color: " . $bg_color . ";'><input class='upc2' id='product_upc-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' title='" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' name='product_upc-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' value='" . html_entity_decode($product['upc'], ENT_QUOTES, 'UTF-8') . "' size='10' /></td>";
					}
					
					$product_html .= "<td class='label-left' style='background-color: " . $bg_color . ";'>";
					...
					if ($this->config->get('config_prod_weight')) {
						...
						$product_html .= "<input style='text-align: right;' class='weight2' type='text' id='weight-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' title='" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' name='weight[" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "]' value='" . (isset($product['weight']) ? number_format($product['weight'] / $product['quantity'], 4, $this->language->get('decimal_point'), $this->language->get('thousand_point')) : 0) . "' size='6' />";
					...
					$product_html .= "<td class='label-center' style='background-color: " . $bg_color . ";'><input style='text-align: center;' class='qty2' type='text' id='quantity-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' title='" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' name='quantity[" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "]' value='" . $cart_qty . "' size='2' /><input type='hidden' id='cunit_qty-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' value='" . (isset($cunit_qty) ? $cunit_qty : 1) . "' /></td>";
					...
					$product_html .= "<td class='label-right' style='background-color: " . $bg_color . ";'><input type='hidden' name='tax' value='" . $product['tax'] . "' /><input style='text-align: right;' class='price2' type='text' id='price-" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' title='" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "' name='price[" . preg_replace("/[^A-Za-z0-9 ]/", '', $product['key']) . "]' value='" . $formated_price . "' size='8' /></td>";
		...
		$balance = $total_data['total'];
		if (isset($this->session->data['payment_method']) && ($this->session->data['payment_method']['code'] != "pending" && $this->session->data['payment_method']['code'] != "pp_link" && $this->session->data['payment_method']['code'] != "invoice" && $this->session->data['payment_method']['code'] != "purchase_order")) {
			$balance = 0;
		}
		$languages = array();
		...
		if ((($this->customer->getBalance() && $this->customer->getBalance() > 0) || ($this->customer->getRewardPoints() || isset($this->session->data['use_reward_points'])) || $this->config->get('config_show_coupon') || $this->config->get('config_show_voucher')) && (!$this->config->get('config_dis_order_credits') || $this->config->get('config_dis_user_group') != $this->user->getId()))  {
			$totals_html .= "<table class='product-table' style='margin-bottom: 35px;'>";
			$totals_html .= "<tbody>";
			$totals_html .= "<tr>";
		...
		if ($this->config->get('config_show_optional') && (!$this->config->get('config_dis_optional_fees')) || $this->config->get('config_dis_user_group') != $this->user->getId()) {
			$totals_html .= "<table class='product-table' style='margin-bottom: 35px;'>";
			$totals_html .= "<tbody>";
			$totals_html .= "<tr>";
		...
		if ($this->config->get('config_show_cart_weight')) {
			...
			if ($this->cart->getWeight()) {
				$cart_weight = number_format($this->cart->getWeight(), 4, $this->language->get('decimal_point'), $this->language->get('thousand_point'));
			} else {
				$cart_weight = 0;
			}
  ...
  $this->load->model('user/user');
    $user_group_id = $this->model_user_user->getUser($this->user->getId());
      ...
      if($user_gid == '12' || $user_gid == '1') {
					...
					if ($payment_method['code'] == "custompayment") {
						if (isset($payment_method['methods'])) {
							foreach ($payment_method['methods'] as $method) {
								if ($method['code'] == $this->session->data['payment_method']['code']) {
			...
			$totals_html .= "</optgroup>";
			$totals_html .= "</select>";
			if (isset($this->session->data['check']) || isset($this->session->data['purchase_order'])) {
    ...
    if(isset($this->session->data['order_paid']) && $this->session->data['order_paid'] == 0) {
      $totals_html .= "<td class='totals-data3' colspan='2'><b>Unpaid Balance</b></td>";
      $totals_html .= "<td class='totals-data1' style='background-color: #FF727D; color: #000000;'><b>" . number_format($total_data['total'], 2, '.','') . "</b></td>";
    }
    else {
  		if ($balance > 0) {
		...
		if (isset($this->session->data['edit_order'])) {
			$order_id = $this->session->data['edit_order'];
			$totals_html .= "<div id='order_buttons2' style='float: left; width: 50%; text-align: center; margin-top: 4px;'>";
			if (isset($this->session->data['quote'])) {
				...
				$totals_html .= "<a class='order_button' rel='Print Packing Skip' title='" . $this->language->get('text_print_packing') . "' alt='" . $this->language->get('text_print_packing') . "' name='" . $order_id . "'><img src='view/image/print_oe2.png' style='border: 0; margin-right: 18px;' /></a>";
			...
			$totals_html .= "</div>";
			$totals_html .= "<div style='float: left; width: 50%;'>";
		...
		$totals_html .= "</table>";
		$totals_html .= "</form>";
		if (!$this->config->get('config_dis_order_history') || $this->config->get('config_dis_user_group') != $this->user->getId()) {
			...
			if (file_exists(DIR_CATALOG . '../vqmod/xml/admin_notes.xml')) {
				...
				$comments_html .= "<td style='border: none !important;'><textarea id='note' name='note' cols='75' rows='4'>" . (isset($this->session->data['note']) ? $this->session->data['note'] : '') . "</textarea></td>";
				$comments_html .= "</tr>";
				$comments_html .= "</table>";
				$comments_html .= "<table class='product-table'>";
				$comments_html .= "<thead>";
				...
				if (isset($this->session->data['edit_order'])) {
					...
					if (!empty($admin_notes) && $admin_notes != "a:0:{}") {
						foreach (unserialize($admin_notes) as $admin_note) {
							$comments_html .= "<tr>";
							$comments_html .= "<td class='data-center'>" . date($this->language->get('date_format_short'), (int)$admin_note['date_added']) . "</td>";
							$comments_html .= "<td class='data-left'>" . $admin_note['author'] . "</td>";
							$comments_html .= "<td class='data-left'>" . $admin_note['note'] . "</td>";
							$comments_html .= "</tr>";
						}
					}
				} else {
					$comments_html .= "<tr>";
					$comments_html .= "<td class='data-center' colspan='3'>" . $this->language->get('text_no_notes') . "</td>";
					$comments_html .= "</tr>";
				}
				$comments_html .= "</tbody>";
				$comments_html .= "</table>";
	...
	private function getCardsaveHosted($total) {
		...
		$fields['State'] = html_entity_decode($this->session->data['payment_address']['zone'], ENT_QUOTES, 'UTF-8');
		...
		$return_html .= "<input type='hidden' name='State' value='" . html_entity_decode($this->session->data['payment_address']['zone'], ENT_QUOTES, 'UTF-8') . "' />";
		...
		if (isset($this->request->get['Paysonref']) && isset($this->request->get['MD5']) && isset($this->request->get['OkURL'])) {
			...
			if ($strMD5Hash == $this->request->get['MD5']) {
				...
				if (isset($this->request->get['RefNr'])) {
					$message .= $this->language->get('text_order_id') . ' / RefNr: ' . $this->request->get['RefNr'] . "\n";
				}
				if ($this->config->get('payson_extra_cost')) {
	...
	private function getTotalWebSecure($total) {
		...
		$zip = html_entity_decode($this->session->data['payment_address']['postcode'], ENT_QUOTES, 'UTF-8');
	...
	private function getPaypalStandard($total) {
		...
		$item_name = html_entity_decode($store_name, ENT_QUOTES, 'UTF-8');
		if (isset($this->session->data['selected_currency'])) {
			$currency_code = $this->session->data['selected_currency']['code'];
		} else {
			$currency_code = $this->config->get('config_currency');
		...
		if (isset($this->session->data['payment_address'])) {
			$first_name = html_entity_decode($this->session->data['payment_address']['firstname'], ENT_QUOTES, 'UTF-8');	
			$last_name = html_entity_decode($this->session->data['payment_address']['lastname'], ENT_QUOTES, 'UTF-8');	
			$address1 = html_entity_decode($this->session->data['payment_address']['address_1'], ENT_QUOTES, 'UTF-8');	
			$address2 = html_entity_decode($this->session->data['payment_address']['address_2'], ENT_QUOTES, 'UTF-8');	
			$city = html_entity_decode($this->session->data['payment_address']['city'], ENT_QUOTES, 'UTF-8');	
			$zip = html_entity_decode($this->session->data['payment_address']['postcode'], ENT_QUOTES, 'UTF-8');	
			$country = $this->session->data['payment_address']['iso_code_2'];
			$invoice = $order_id . ' - ' . html_entity_decode($this->session->data['payment_address']['firstname'], ENT_QUOTES, 'UTF-8') . ' ' . html_entity_decode($this->session->data['payment_address']['lastname'], ENT_QUOTES, 'UTF-8');
		} else {
			$first_name = '';	
			$last_name = '';	
			$address1 = '';	
	...
	private function getShippingMethods($total) {
		...
		if ($results) {
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status')) {
					$this->language->load('oentryshipping/' . $result['code']);
	...
	private function getPaymentMethods($total) {
		...
		$method_data['pending'] = array( 
			'code'			=> 'pending',
			'title'			=> 'Pending', //$this->language->get('text_pending_payment'),
			'sort_order'	=> 99994
		);
...
*/
	  $method_data['invoiced'] = array(
      'code'      => 'invoiced',
      'title'     => 'Invoice', //$this->language->get('text_cc_offline'),
      'sort_order'  => 99992
    );
