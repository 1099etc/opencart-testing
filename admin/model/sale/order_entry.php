...
class ModelSaleOrderEntry extends Model {
	...
	public function editOrder($order_id, $data) {
		...
		if (isset($data['invoice_number'])) {
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET invoice_number = '', invoice_date = '0000-00-00 00:00:00' WHERE order_id = '" . (int)$order_id . "'");
		}
	...
	public function autocomplete($data, $type) {
		...
		if ($type == "customer") {
			...
			if ($disabled) {
				if ($contains) {
					$query = $this->db->query("SELECT p.product_id, p.upc FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2s.store_id = '" . (int)$store_id . "' AND LCASE(p.upc) LIKE '%" . $this->db->escape(mb_strtolower($data['upc'], 'UTF-8')) . "%' ORDER BY p.upc LIMIT " . $data['start'] . "," . $data['limit']);
				} else {
					$query = $this->db->query("SELECT p.product_id, p.upc FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2s.store_id = '" . (int)$store_id . "' AND LCASE(p.upc) LIKE '" . $this->db->escape(mb_strtolower($data['upc'], 'UTF-8')) . "%' ORDER BY p.upc LIMIT " . $data['start'] . "," . $data['limit']);
				}
			} else {
				if ($contains) {
					$query = $this->db->query("SELECT p.product_id, p.upc FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2s.store_id = '" . (int)$store_id . "' AND p.status = 1 AND LCASE(p.upc) LIKE '%" . $this->db->escape(mb_strtolower($data['upc'], 'UTF-8')) . "%' ORDER BY p.upc LIMIT " . $data['start'] . "," . $data['limit']);
				} else {
					$query = $this->db->query("SELECT p.product_id, p.upc FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2s.store_id = '" . (int)$store_id . "' AND p.status = 1 AND LCASE(p.upc) LIKE '" . $this->db->escape(mb_strtolower($data['upc'], 'UTF-8')) . "%' ORDER BY p.upc LIMIT " . $data['start'] . "," . $data['limit']);
				}
			}
			...
			if ($disabled) {
				if ($contains) {
					$query = $this->db->query("SELECT p.product_id, p.model FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2s.store_id = '" . (int)$store_id . "' AND LCASE(p.model) LIKE '%" . $this->db->escape(mb_strtolower($data['model'], 'UTF-8')) . "%' ORDER BY p.model LIMIT " . $data['start'] . "," . $data['limit']);
				} else {
					$query = $this->db->query("SELECT p.product_id, p.model FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2s.store_id = '" . (int)$store_id . "' AND LCASE(p.model) LIKE '" . $this->db->escape(mb_strtolower($data['model'], 'UTF-8')) . "%' ORDER BY p.model LIMIT " . $data['start'] . "," . $data['limit']);
				}
			} else {
				if ($contains) {
					$query = $this->db->query("SELECT p.product_id, p.model FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2s.store_id = '" . (int)$store_id . "' AND p.status = 1 AND LCASE(p.model) LIKE '%" . $this->db->escape(mb_strtolower($data['model'], 'UTF-8')) . "%' ORDER BY p.model LIMIT " . $data['start'] . "," . $data['limit']);
				} else {
					$query = $this->db->query("SELECT p.product_id, p.model FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2s.store_id = '" . (int)$store_id . "' AND p.status = 1 AND LCASE(p.model) LIKE '" . $this->db->escape(mb_strtolower($data['model'], 'UTF-8')) . "%' ORDER BY p.model LIMIT " . $data['start'] . "," . $data['limit']);
				}
			}
	...
	public function alterOrderTable() {
		...
		if ($results->num_rows < 1) {
			$this->db->query("ALTER TABLE `" . DB_PREFIX . "order` ADD COLUMN cart_weight decimal(5,2) NOT NULL DEFAULT 0.00");
		}
		...
		if ($results->num_rows < 1) {
			$this->db->query("ALTER TABLE `" . DB_PREFIX . "order_product` ADD COLUMN weight decimal(5,2) NOT NULL DEFAULT 0.00");
		}
