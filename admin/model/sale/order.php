...
class ModelSaleOrder extends Model {
	public function addOrder($data) {
		...
		if ($currency_info) {
      	...
      	if(!isset($data['shipping_description'])) { $data['shipping_description'] = ''; }
  ...
  public function getOptionPriceByIDandName($product_option_id, $name) {
        $query = $this->db->query("SELECT " . DB_PREFIX . "product_option_value.price FROM " . DB_PREFIX . "product_option_value, " . DB_PREFIX . "option_value_description WHERE " . DB_PREFIX . "product_option_value.option_value_id = " . DB_PREFIX . "option_value_description.option_value_id and " . DB_PREFIX . "product_option_value.product_option_id = '" . (int)$product_option_id . "' and " . DB_PREFIX . "option_value_description.name = '" . $name . "'" );
...
//		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$order_product_id . "'");
  ...
  $query = $this->db->query("SELECT  " . DB_PREFIX . "order_option.product_option_value_id, " . DB_PREFIX . "product_option_value.product_option_id, " . DB_PREFIX . "order_option.type, " . DB_PREFIX . "order_option.name, " . DB_PREFIX . "order_option.value, " . DB_PREFIX . "product_option_value.price FROM " . DB_PREFIX . "order_option, " . DB_PREFIX . "product_option_value WHERE " . DB_PREFIX . "order_option.order_id='" . (int)$order_id . "' AND " . DB_PREFIX . "order_option.order_product_id = '" . (int)$order_product_id . "' and " . DB_PREFIX . "product_option_value.product_option_value_id = " . DB_PREFIX . "order_option.product_option_value_id");
	...
	public function createInvoiceNo($order_id) {
		...
		if ($order_info && !$order_info['invoice_no']) {
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET invoice_no = '" . (int)$invoice_no . "', invoice_prefix = '" . $this->db->escape($order_info['invoice_prefix']) . "', invoice_date = CURRENT_TIMESTAMP() WHERE order_id = '" . (int)$order_id . "'");
...
//    $this->cache->delete('product');
    ...
    // Gift Voucher
    $this->load->model('../catalog/checkout/voucher');
    ...
    //$language->load('../catalog/' . $order_info['language_filename']);
    ...
    $subject = sprintf('%s - Order %s', $order_info['store_name'], $order_id);

    $template->data['title'] = sprintf('%s - Order %s', html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'), $order_id);
    $template->data['text_greeting'] = "";
    $template->data['text_link'] = 'To view your order click on the link below:';
    if($order_info['order_paid'] == 0 )
    {
      $template->data['text_download'] = 'Once your payment has been confirmed you can click on the link below to access your downloadable products:';
    }
    else
    {
      $template->data['text_download'] = 'You can click on the link below to access your downloadable products:';
    }
    $template->data['text_order_detail'] = 'Order Details';
    $template->data['text_instruction'] = 'Instructions';
    $template->data['text_order_id'] = 'Order ID:';
    $template->data['text_date_added'] = 'Date Added:';
    $template->data['text_payment_method'] = 'Payment Method:';
    $template->data['text_po_number'] = 'Purchase Order:';
    $template->data['text_shipping_method'] = 'Shipping Method:';
    $template->data['text_email'] = 'Email:';
    $template->data['text_telephone'] = 'Telephone:';
    $template->data['text_ip'] = 'IP Address:';
    $template->data['text_payment_address'] = 'Payment Address';
    $template->data['text_shipping_address'] = 'Shipping Address';
    $template->data['text_product'] = 'Product';
    $template->data['text_model'] = 'Model';
    $template->data['text_quantity'] = 'Quantity';
    $template->data['text_price'] = 'Price';
      $template->data['text_total'] = 'Total';
    $template->data['text_footer'] = "";
    $template->data['text_powered'] = "";
//    $template->data['logo'] = $this->config->get('config_url') . '../image/' . $this->config->get('config_logo');
    $template->data['logo'] = 'http://shop.1099-etc.com/image/data/logo.png';
    $template->data['store_name'] = $order_info['store_name'];
    $template->data['store_url'] = $order_info['store_url'];
    $template->data['customer_id'] = $order_info['customer_id'];
    $template->data['link'] = $order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_id;

    $template->data['AlertMessage'] = $AlertMessage;
    ...
    $template->data['po_number'] = $order_info['po_number'];
    ...
    /*foreach ($this->getSetting('data') as $row_num => $row) {
      ...
      {
        $template->data['shipping_method'] .= '<br>'.html_entity_decode($row['description']);
      }
    }*/
    $find = array(
      '{firstname}',
    ...
    //$this->load->model('../catalog/serial');
    $template->data['text_serial_keys'] = 'Serial numbers and key codes for this product:';
    ...
    foreach ($order_product_query->rows as $product) {
      ...
      if($order_info['order_status_id'] == $this->config->get('config_complete_status_id')) {
        ...
        if(is_array($featurecodes)) { $featurecodes = array_values($featurecodes); }
        ...
        if(is_array($sns)) {
          ...
          if(!empty($sns)) {
            $serials[] = "Serial Number: " . $sns . $featurecodes . " | Keycode: " . $this->model_catalog_serial->generate_keycode($sns . $featurecodes);
          }
        }
      }
    ...
    $totals_data = $order_total_query->rows;
    foreach ($totals_data as $key=>$total) {
        if($total['title'] == 'Total' && $order_info['order_paid'] == 0)
        {
          $totals_data[$key]['title'] = 'Unpaid Balance';
        }
        elseif ($total['title'] == 'Total')
        {
          $totals_data[$key]['title'] = 'Total Paid';
        }
      }
    if($order_info['order_paid'] == 0 ) {
          foreach($totals_data as $t => $total) {
            if($total['code'] == 'total') {
              $unpaidBalanceText  = $total['text'];
              $unpaidBalanceValue = $total['value'];
              $total_data[] = array(
                'order_id' => $order_id,
                'code'     => 'unpaid_balance',
                'title'    => 'Unpaid Balance',
                'text'     => $unpaidBalanceText,
                'value'    => $unpaidBalanceValue,
                'sort_order' => 50
              );
              $u = $t;
            }
    ...
    $template->data['totals'] = $totals_data;

    if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '../../../catalog/view/theme/amsTheme/template/mail/order.tpl')) {
      $html = $template->fetch($this->config->get('config_template') . '../../../catalog/view/theme/amsTheme/template/mail/order.tpl');
    } else {
    ...
    if(isset($data['override']) && strlen(trim($data['override'])) > 0)
    {
